...
class Optimizer:
    ...
    def get_weights_and_features(self):
        ...
        try:
            ...
            w_paakk = np.arange(df['paa_kk'].min(), df['paa_kk'].max(), (df['paa_kk'].max()-df['paa_kk'].min())/50)
            w_paaf = np.arange(df['paa_f'].min(), df['paa_f'].max(), (df['paa_f'].max()-df['paa_f'].min())/50)
            ...
            weights_combo = pd.DataFrame(data=combos, columns=['paa_kk', 'paa_f', 'sa', 'permanganate'])

            single_features_weights = df_feature.iloc[0, :-4].to_frame().T.merge(weights_combo, how='cross')
